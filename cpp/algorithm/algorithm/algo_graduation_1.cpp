...
int prerequisite[13], //선수 과목 
courses[11]; //학기 개설 과목 
int N, //전공과목 수
K, //들어야할 과목 수 
M; //한학기에 들을 수 있는 최대 과목 수 


int dp[11][5000]; //dp[학기][수강상태] = 앞으로 들어야할 최소 학기 수
int minGraduationCondition(int semesterNo, int taken) {
	if (__popcnt(taken) >= K) return 0; //모든 과목 수강
...
void solve() {
	...
	cout << minGraduationCondition(0, 0) << endl;
}
